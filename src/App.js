...
class App extends Component {
  ...
  handleClose = () => {
    if (
      (this.state.name && this.state.link) ||
      (this.state.name !== "" && this.state.link !== "")
    ) {
      this.setState({ open: false });
      this.submitData();
  ...
  componentDidMount() {
    var that = this;
    var url = "http://mumineenapp.com/api/fetch/fetch_links.php";
    var data = {};
  ...
  render() {
    const actions = [
      ...
      <FlatButton label="Submit" primary={true} onClick={this.close} />
    ];
    ...
    return (
      <MuiThemeProvider>
        <div className="rootContainer">
          <AppBar
            className="appbar"
            title={<span style={styles.title}> Links for Mumineen </span>}
          />
          <FloatingActionButton
          ...
          >
            <span className="modalDesc">
              Suggest a link that is missing from this list
            </span>
            <TextField
                  ...
                  <a target="_blank" href={item.link}>
                    <ListItem
                      leftAvatar={
                        <Avatar
                          className="avtar"
                          src={item.img || "chrome.png"}
                        />
                      }
                      primaryText={
                        <span className="itemTitle"> {item.name} </span>
                      }
                      secondaryText={<p> {item.purpose} </p>}
          ...
          <List>
            ...
            {this.state.instaLinks.map(item => {
              return (
                <div key={item.id} className="listItem">
